-- RECRIAÇÃO SEGURA: Moderador com acesso igual a Admin
-- Versão simplificada e funcional

-- ============================================
-- 1. CRIAR FUNÇÕES HELPER (fora do bloco DO)
-- ============================================
...
-- ============================================
-- 2. FUNÇÃO PARA RECRIAR POLICIES
-- ============================================
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'clients' AND table_schema = 'public') THEN
    ...
    CREATE POLICY "clients_admin_moderador" ON clients FOR ALL TO authenticated USING (is_admin_or_moderador());
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'goals' AND table_schema = 'public') THEN
    ...
    CREATE POLICY "goals_own" ON goals FOR ALL USING (EXISTS (SELECT 1 FROM clients WHERE clients.id = goals.client_id AND clients.profile_id = auth.uid()));
    CREATE POLICY "goals_admin_moderador" ON goals FOR ALL TO authenticated USING (is_admin_or_moderador());
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'goal_progress' AND table_schema = 'public') THEN
    ...
    CREATE POLICY "goal_progress_own" ON goal_progress FOR ALL USING (EXISTS (SELECT 1 FROM goals g JOIN clients c ON c.id = g.client_id WHERE g.id = goal_progress.goal_id AND c.profile_id = auth.uid()));
    CREATE POLICY "goal_progress_admin_moderador" ON goal_progress FOR ALL TO authenticated USING (is_admin_or_moderador());
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'sales_history' AND table_schema = 'public') THEN
    ...
    CREATE POLICY "sales_history_own" ON sales_history FOR ALL USING (EXISTS (SELECT 1 FROM clients WHERE clients.id = sales_history.client_id AND clients.profile_id = auth.uid()));
    CREATE POLICY "sales_history_admin_moderador" ON sales_history FOR ALL TO authenticated USING (is_admin_or_moderador());
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'meta_cache' AND table_schema = 'public') THEN
    ...
    CREATE POLICY "meta_cache_own" ON meta_cache FOR ALL USING (EXISTS (SELECT 1 FROM clients WHERE clients.id = meta_cache.client_id AND clients.profile_id = auth.uid()));
    CREATE POLICY "meta_cache_admin_moderador" ON meta_cache FOR ALL TO authenticated USING (is_admin_or_moderador());
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'app_settings' AND table_schema = 'public') THEN
    ...
    CREATE POLICY "app_settings_admin_moderador" ON app_settings FOR ALL TO authenticated USING (is_admin_or_moderador());
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ai_agents' AND table_schema = 'public') THEN
    ...
    CREATE POLICY "ai_agents_public_read" ON ai_agents FOR SELECT USING (active = true);
    CREATE POLICY "ai_agents_admin_moderador" ON ai_agents FOR ALL TO authenticated USING (is_admin_or_moderador());
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'bookings' AND table_schema = 'public') THEN
    ...
    CREATE POLICY "bookings_own" ON bookings FOR ALL USING (EXISTS (SELECT 1 FROM clients WHERE clients.id = bookings.client_id AND clients.profile_id = auth.uid()));
    CREATE POLICY "bookings_admin_moderador" ON bookings FOR ALL TO authenticated USING (is_admin_or_moderador());
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'booking_blocked_slots' AND table_schema = 'public') THEN
    ...
    CREATE POLICY "booking_blocked_slots_public_read" ON booking_blocked_slots FOR SELECT USING (true);
    CREATE POLICY "booking_blocked_slots_admin_moderador" ON booking_blocked_slots FOR ALL TO authenticated USING (is_admin_or_moderador());
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'booking_window_config' AND table_schema = 'public') THEN
    ...
    CREATE POLICY "booking_window_config_public_read" ON booking_window_config FOR SELECT USING (true);
    CREATE POLICY "booking_window_config_admin_moderador" ON booking_window_config FOR ALL TO authenticated USING (is_admin_or_moderador());
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_client_access' AND table_schema = 'public') THEN
    ...
    CREATE POLICY "user_client_access_own" ON user_client_access FOR SELECT USING (user_id = auth.uid());
    CREATE POLICY "user_client_access_admin_moderador" ON user_client_access FOR ALL TO authenticated USING (is_admin_or_moderador());
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'pending_registrations' AND table_schema = 'public') THEN
    ...
    CREATE POLICY "pending_registrations_admin_moderador" ON pending_registrations FOR ALL TO authenticated USING (is_admin_or_moderador());
  END IF;
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'gestores_nicho' AND table_schema = 'public') THEN
    ...
    CREATE POLICY "gestores_nicho_admin_moderador" ON gestores_nicho FOR ALL TO authenticated USING (is_admin_or_moderador());
  END IF;
