-- MODERADOR COM PODERES DE ADMIN COMPLETO (VERSÃO FINAL)
...
-- Booking blocked dates (tabela antiga, mas pode existir)
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'booking_blocked_dates') THEN
...
-- Booking blocked slots (tabela atual)
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'booking_blocked_slots') THEN
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'booking_window_config') THEN
