-- MODERADOR COM PODERES DE ADMIN COMPLETO
...
-- ============================================
-- 2. POLICIES PARA CLIENTES (já existem, garantir que estão corretas)
-- ============================================
...
-- ============================================
-- 3. POLICIES PARA AGENDAMENTO (configurar agenda)
-- ============================================

-- Agendamentos
DROP POLICY IF EXISTS "agendamentos_own" ON agendamentos;
DROP POLICY IF EXISTS "agendamentos_gestores" ON agendamentos;
DROP POLICY IF EXISTS "moderadores_all_access_agendamentos" ON agendamentos;
DROP POLICY IF EXISTS "admins_moderadores_all_agendamentos" ON agendamentos;

CREATE POLICY "admins_moderadores_all_agendamentos" ON agendamentos
  FOR ALL
  TO authenticated
  USING (
    (SELECT role FROM profiles WHERE id = auth.uid()) IN ('admin', 'moderador')
...
DROP POLICY IF EXISTS "moderadores_all_access_booking_blocked" ON booking_blocked_dates;
...
DROP POLICY IF EXISTS "moderadores_all_access_booking_slots" ON booking_slots;
