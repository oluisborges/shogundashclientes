...
export function DateRangePicker({ value, onChange, className, isOpen: externalIsOpen }: DateRangePickerProps) {
  ...
  const renderCalendar = (monthOffset: number = 0) => {
    ...
    return (
      <div className="flex-1">
        <div className="text-center font-[var(--font-display)] font-semibold text-shogun-text-primary mb-2 text-xs">
          {MONTHS[displayMonth.getMonth()]} {displayMonth.getFullYear()}
        </div>
        <div className="grid grid-cols-7 gap-0.5 mb-1">
          {WEEKDAYS.map((day, i) => (
            <div key={i} className="text-center text-[10px] text-shogun-text-muted font-[var(--font-display)] font-medium">
              {day}
            </div>
          ))}
        </div>
          ...
          {days.map((date, i) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "aspect-square rounded text-xs font-[var(--font-display)] transition-colors relative",
